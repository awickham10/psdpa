    ...
    InModuleScope 'PSDPA' {
        ...
        Mock -CommandName 'Invoke-RestMethod' -MockWith {
            ...
            elseif ($Uri -like '*/databases/*/monitor-status' -and $Method -eq 'PUT') {
                ...
                if ($json.command -eq 'START') {
                    Set-Content -Path 'TestDrive:\Monitor\Mock1State.txt' -Value 'Started'
                }
            }
            else {
        ...
        It 'starts a monitor by -DatabaseId' {
            $databaseId = 1
            Start-DpaMonitor -DatabaseId $databaseId
            Get-Content -Path 'TestDrive:\Monitor\Mock1State.txt' | Should -Be 'Started'
        ...
        It 'starts a monitor by -MonitorName' {
            $monitorName = 'MOCK-1'
            Start-DpaMonitor -MonitorName $monitorName
            Get-Content -Path 'TestDrive:\Monitor\Mock1State.txt' | Should -Be 'Started'
        ...
        It 'starts a monitor by -Monitor' {
            Start-DpaMonitor -Monitor $monitor
            Get-Content -Path 'TestDrive:\Monitor\Mock1State.txt' | Should -Be 'Started'
        ...
        It 'starts a monitor from the pipeline' {
            $monitor | Start-DpaMonitor
            Get-Content -Path 'TestDrive:\Monitor\Mock1State.txt' | Should -Be 'Started'

            Assert-MockCalled -CommandName 'Invoke-RestMethod' -Times 1
        }

        It 'does not throw an exception if the monitor could not be started and -EnableException is not used' {
            <#
            Need to figure out how to test this
            #>
        }

        It 'throws an exception if the monitor could not be started and -EnableException is used' {
            <#
            Need to figure out how to test this
            #>
        }
